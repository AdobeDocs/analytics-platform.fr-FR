...
3. Configurez maintenant les jeux de données un par un.
   ...
   | **[!UICONTROL Importer de nouvelles données]** | Définissez cette option sur [!UICONTROL Activé] si vous souhaitez établir une connexion. Lorsque cette option est sélectionnée, les nouveaux lots de données qui sont ajoutés aux jeux de données dans cette connexion sont automatiquement transférés dans Workspace. |
