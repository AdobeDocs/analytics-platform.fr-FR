...
>title="Activer l’assemblage d’identités"
>abstract="L’activation de la combinaison d’identités extrait les informations d’identité utilisateur d’un jeu de données ou du graphique d’identités. Ce processus peut inclure la fusion de données d’utilisation provenant de sessions authentifiées et non authentifiées.<br/><br/>Il vous incombe de vous conformer aux lois et règlements applicables. Cette conformité implique que vous obteniez les autorisations d’utilisateur final nécessaires avant d’activer le groupement sur un jeu de données."
>additional-url="https://experienceleague.adobe.com/fr/docs/analytics-platform/using/stitching/overview" text="Vue d’ensemble du groupement"
