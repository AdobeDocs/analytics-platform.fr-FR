...
description: Décrit comment gérer des connexions aux jeux de données Experience Platform dans Customer Journey Analytics (Customer Journey Analytics).
mini-toc-levels: 3
...
| **[!UICONTROL Schéma]** | Schéma Experience Platform sur lequel le jeu de données est basé. |
...
| **[!UICONTROL Sandbox]** | Le [sandbox Experience Platform](https://experienceleague.adobe.com/fr/docs/experience-platform/sandbox/home) à partir duquel cette connexion tire son ou ses jeux de données. Ce sandbox a été sélectionné lorsque vous avez créé la connexion pour la première fois. Il ne peut pas être modifié. |
...
Cette interface vous aide à déterminer si votre utilisation de Customer Journey Analytics est conforme à ce qui est convenu contractuellement. Outre la surveillance, vous pouvez utiliser l’interface d’utilisation pour planifier le renouvellement de votre licence Customer Journey Analytics.
