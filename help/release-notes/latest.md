...
| **Prise en charge de la mise en miroir des données** | Grâce à la prise en charge des schémas basés sur des modèles et de la fonctionnalité de capture de données modifiées (CDC) pour des connecteurs sources spécifiques dans Experience Platform, Customer Journey Analytics sera en mesure de prendre en charge la fonctionnalité de mise en miroir des données des solutions d’entrepôt de données telles que [!DNL Snowflake], [!DNL Azure Databricks] et [!DNL Google BigQuery].<p>Contactez votre équipe Adobe en charge des comptes pour accéder à la version Beta.</p><p>(Lien vers la documentation à suivre.)</p> | Version bêta : 24 septembre 2025 | À confirmer |
