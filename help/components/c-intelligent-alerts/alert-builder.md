...
1. Dans Customer Journey Analytics, <!-- add this back in after the other methods are available like in AA and make a bulleted list: "You can access the alert builder in any of the following ways:" -->, sélectionnez **[!UICONTROL Composants]** > **[!UICONTROL Alertes]**. Dans le [gestionnaire d’alertes](alert-manager.md), sélectionnez ![Ajouter un cercle](/help/assets/icons/AddCircle.svg) **** pour créer une alerte ou sélectionnez l’une des alertes répertoriées pour modifier une alerte existante.

   L’interface du [Générateur d’alertes](#alert-builder) s’affiche.
...
Les utilisateurs qui ont déjà créé des filtres ou calculé des mesures dans le Customer Journey Analytics ne seront pas dépaysés par l’interface du Générateur d’alertes :
