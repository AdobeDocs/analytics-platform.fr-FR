...
Identifiant numérique qui représente le navigateur.
...
* **Modèle d’appareil** : `xdm.environment.browserDetails.userAgentClientHints.model`
* **Bitness** : `xdm.environment.browserDetails.userAgentClientHints.bitness`
* **Fournisseur du navigateur** : `xdm.environment.browserDetails.userAgentClientHints.vendor`
...
Cette colonne ne s’applique pas, car Customer Journey Analytics ne prend pas encore en charge Activity Map.
...
Identifiant numérique représentant la dimension Type de connexion .
...
Customer Journey Analytics ne dispose pas d’un concept natif de type d’événement dans lequel il inclut ou exclut automatiquement les accès en fonction du contexte de l’accès. Vous pouvez utiliser `xdm.eventType` pour déterminer les événements à inclure et à exclure dans la plupart des rapports.
...
Si votre organisation dispose d’une plateforme de gestion des données, elle remplit probablement les champs XDM souhaités pour cette dimension.
...
Vous pouvez utiliser `xdm.timestamp` et appliquer le paramètre du composant **[!UICONTROL Date]** ou **[!UICONTROL Date-heure]** [Format](/help/data-views/component-settings/format.md).
...
Activez la **[!UICONTROL recherche réseau]** lors de la [configuration d’un flux de données](https://experienceleague.adobe.com/fr/docs/experience-platform/datastreams/configure). Le champ XDM est `xdm.environment.domain` s’il est inclus dans votre schéma.
...
Cette colonne ne s’applique pas, car Customer Journey Analytics ne dispose pas d’un concept de règles VISTA.
...
* **Ouvertures de panier** : `xdm.commerce.productListOpens.value`
* **Retraits du panier** : `xdm.commerce.productListRemovals.value`
* **Vues du panier** : `xdm.commerce.productListViews.value`
* **Consultations de produit** : `xdm.commerce.productViews.value`
* **Commandes** : `xdm.commerce.purchases.value`
...
Les accès exclus n’existent plus dans les rapports Customer Journey Analytics, mais sont toujours disponibles dans les exportations de flux de données.
...
Identifiant numérique représentant la langue du visiteur ou de la visiteuse.
...
Identifiant numérique représentant le système d’exploitation du visiteur ou de la visiteuse. Basé sur la colonne `user_agent`.

{{cja-df-lookup}}

Lors de la [configuration d’un flux de données](https://experienceleague.adobe.com/fr/docs/experience-platform/datastreams/configure), vous pouvez activer la **[!UICONTROL recherche d’appareil]**. Si cette option est activée, cochez la case **[!UICONTROL Système d’exploitation]**. Les champs XDM suivants sont alors automatiquement renseignés si vous avez inclus ces champs dans votre schéma :

* **Fournisseur du système d’exploitation** : `xdm.environment.operatingSystemVendor`
* **Nom du système d’exploitation** : `xdm.environment.operatingSystem`
* **Version du système d’exploitation** : `xdm.environment.operatingSystemVersion`
...
Identifiant numérique représentant la résolution du moniteur. Utilisé dans la dimension Résolution du moniteur .
...
Identifiant numérique représentant le moteur de recherche qui a renvoyé le visiteur ou la visiteuse sur votre site. Utilisé dans les dimensions du moteur de recherche.
...
Identifiant numérique qui identifie la dimension du canal Dernière touche .
...
Identifiant numérique qui identifie la dimension du canal Première touche.
