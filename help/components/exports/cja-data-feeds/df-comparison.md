...
| **Gestion des sous-événements**<br/> Représentation des sous-événements dans la sortie du flux de données. | Représenté sur une seule ligne, mais la hiérarchie relationnelle est conservée. Pour plus d’informations, voir [ Sous-événements dans les flux de données ](/help/components/exports/cja-data-feeds/df-sub-event.md). | Représenté sur une seule ligne sous la forme d’une chaîne aplatie et délimitée. L’analyse de la chaîne nécessite une logique personnalisée. |
