...
| Recettes / Visiteur | Montant moyen des recettes générées par chaque personne sur le site. | `[Revenue] / [Unique Visitors]` |
| Commandes / Visiteur | Nombre moyen de commandes ou transactions générées par chaque personne sur le site | `[Orders] / [Unique Visitors]` |
...
| Visites / Visiteur | Nombre moyen de visites qu’une personne unique effectue sur le site. | `[Visits] / [Unique Visitors]` |
...
| Taux de sortie | Pourcentage de personnes qui quittent le site après avoir consulté une page spécifique. | `[Exits] / [Visits]` |
| Taux d’entrée | Le pourcentage de personnes qui sont entrées sur le site sur une page donnée, par rapport au nombre total de sessions sur le site. | `[Entries] / [Visits]` |
| Durée moyenne du site | Durée moyenne passée par une personne sur le site avant de quitter le site ou de quitter le site. | `[Average Time Spent on Site (Seconds)]` |
| Durée de la visite/de l’utilisateur (état) | La durée passée par la personne moyenne dans un état particulier sur le site | `[Mobile App Users] (filter)`<br>`[Time Spent per Visitor (Seconds)] (metric)` |
...
| Taux de conversion | Pourcentage de personnes qui ont effectué une action souhaitée, par exemple effectué un achat. | `[Orders] / [Visits]` |
| Durée de session moyenne (mobile) | Durée moyenne passée par les personnes sur le site au cours d’une seule session. | Vide |
| Couverture Experience Cloud ID | Pourcentage de personnes possédant un identifiant Experience Cloud. | `[Visitors with Experience Cloud ID] / [Unique Visitors]` |
...
| Visiteurs uniques ITP 2.1 / Visiteurs uniques | Pourcentage de personnes uniques utilisant un navigateur affecté par les limitations des cookies ITP 2.1. En d’autres termes, les clients n’utilisant pas de mise en oeuvre CNAME. (Cela s’applique aux clients qui définissent des cookies via JavaScript côté client.) | `[Unique Visitors metric with ITP Visitors filter] / [Unique Visitors]` |
| Visiteurs uniques / Visiteurs uniques revenant après 7 jours | Pourcentage de personnes uniques qui reviennent après une période de 7 jours ou plus. | `[Unique Visitors metric with Users returning after 7 days filter] / [Unique Visitors]` |
| Pages vues / Visiteur unique | Le nombre moyen de pages vues pour chaque personne unique sur le site. | `[Page Views] / [Unique Visitors]` |
| Visites/Visiteurs et visiteuses | Nombre moyen de visites qu’une personne unique effectue sur le site . | `[Visits] / [Unique Visitors]` |
| Nombre estimé de visiteurs uniques (ITP 2.1) | Pour les personnes ITP (utilisateurs sur les navigateurs Safari), divisez les visiteurs uniques par 2 ou moins. Cette mesure calculée suppose que vous définissez des cookies à l’aide de code JavaScript côté client (sans utiliser d’implémentation CNAME). Les mises en oeuvre qui définissent des cookies à l’aide de JavaScript côté client ont été affectées à partir d’ITP 2.1. Voir [Prévention intelligente du suivi](https://webkit.org/blog/8613/intelligent-tracking-prevention-2-1/) pour plus d’informations. | `[Unique Visitors (metric) with ITP Visitors (ITP 2.1, Non-CNAME implementations) filter] / [Unique Visitors metric + Non-ITP Visitors (ITP 2.1, Non-CNAME implementations) filter]` |
