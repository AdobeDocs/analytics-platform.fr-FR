...
| **Utiliser l’[API d’annotations Customer Journey Analytics](https://developer.adobe.com/cja-apis/docs/endpoints/annotations/)** | Les API d’annotations Customer Journey Analytics vous permettent de créer, mettre à jour ou récupérer des annotations par programmation via Adobe Developer. Ces API utilisent les mêmes données et méthodes que celles utilisées par Adobe dans l’interface utilisateur du produit. |
