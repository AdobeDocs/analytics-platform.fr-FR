...
description: Activez le groupement pour les jeux de données d’événements dans Customer Journey Analytics. Configurez les identifiants persistants, les identifiants de personne et les fenêtres de relecture dans l’interface utilisateur de Connexions.
...
>title="Couverture d’ID de personne"
>abstract="La couverture de l’ID de personne sélectionné utilisé pour l’identification pendant le processus d’assemblage (en direct et en relecture).<br/>Pour des résultats d’assemblage optimaux, une relation (ID persistant, ID de personne) doit être présente dans le graphique d’identité pour chaque ID persistant."

>[!CONTEXTUALHELP]
>id="connection_stitchingmetrics_fbs_personidcoverage"
>title="Couverture d’ID de personne"
>abstract="Couverture de l’ID de personne sélectionné utilisé pour l’identification pendant le processus d’assemblage (en direct et en relecture). <br/>Pour des résultats d’assemblage optimaux, l’ID de personne (informations d’utilisation) doit être envoyé sur au moins un événement pour chaque ID persistant (informations sur l’appareil)."
...
![Options de combinaison d’identités lorsque vous activez la fonction](assets/identity-stitching-ui-preview.png)

#### Mesures de l’assemblage

>[!AVAILABILITY]
>
>Les mesures de groupement ne sont pas disponibles pour le groupement basé sur les graphiques.
>
...
>[!AVAILABILITY]
>
>Les ID incorrects ne sont pas disponibles pour le groupement basé sur les graphiques.
>
...
Le groupement activé dans l’interface de connexion peut coexister sans problème avec le groupement basé sur les requêtes.
