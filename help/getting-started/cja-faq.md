...
| Et si je... | Ça arrive... |
| --- | --- |
| Suppression d’un sandbox dans [!UICONTROL Adobe Experience Platform]? | La suppression d’un sandbox interrompt le flux de données vers n’importe quel [!UICONTROL Customer Journey Analytics] se connecte à des jeux de données dans ce sandbox. Actuellement, les connexions dans CJA liées à ce sandbox ne seront pas automatiquement supprimées. |
