...
description: Éléments à prendre en compte du point de vue de l’utilisateur lorsque votre entreprise déplace des données d’Adobe Analytics vers Customer Journey Analytics
role: User
...
* [Report Builder](/help/report-builder/report-buider-overview.md) a une nouvelle interface et fonctionne sur MS Windows, iOS et la version Web d’Excel. (Avant cette version de Report Builder, vous ne pouviez pas l’utiliser sur Mac à moins de l’exécuter sur Vmware.) Cette version ne prend pas encore en charge les demandes de données AA traditionnelles.
...
* **Les conteneurs d’accès sont désormais des conteneurs Événéments**. Le conteneur [!UICONTROL Personne] inclut chaque session et événement pour les visiteurs au cours dʼune période indiquée.
* **Les conteneurs Visiteur sont désormais des conteneurs Session**. Le conteneur [!UICONTROL Session] permet dʼidentifier les interactions de pages, les campagnes ou les conversions pour une session spécifique.
