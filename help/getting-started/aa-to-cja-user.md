...
Les concepts de [!UICONTROL eVars], [!UICONTROL props], et [!UICONTROL events] dans l’Adobe Analytics classique n’existe plus dans [!UICONTROL Customer Journey Analytics]. Dans Adobe Analytics, les eVars et les props stockent des descriptions de contenu, de clients, de campagnes, etc. Les événements et comptent des éléments tels que les recettes, les abonnements ou les pistes générées. Customer Journey Analytics conserve les deux types de données et vous pouvez y accéder de la même manière, depuis le rail de gauche dans Analysis Workspace, sous Dimensions ou Mesures, respectivement.

Dans CJA, un nombre illimité d’éléments de schéma sont disponibles, y compris les dimensions, les mesures et les champs de liste. Ils sont mappés à un nombre illimité d’éléments de schéma, y compris des dimensions, des mesures et des champs de liste dans Experience Platform. Tous les paramètres de visite et d’attribution appliqués après les règles de traitement dans Adobe Analytics s’appliquent désormais au moment de la requête dans Customer Journey Analytics.

Avec cette flexibilité, vous pouvez rencontrer des situations dans lesquelles un seul champ de schéma peut être utilisé comme à la fois comme dimensions et comme une mesure pour prendre en charge différents besoins de suivi.
