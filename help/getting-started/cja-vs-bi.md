...
« Progressivement » est ici un concept clé : contrairement à la plupart des visualisations dans les outils de BI, l’interface d’utilisation visuelle par glisser-déposer de Customer Journey Analytics vous permet de ventiler en continu vos données en fonction de vos besoins. Vous pouvez créer des requêtes visuelles de manière interactive à l’aide de mesures, dimensions, segments, calculs, chronologies, annotations et autres valeurs d’analyse pertinentes.
