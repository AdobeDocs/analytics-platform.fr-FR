...
Grâce à l’optimisation de la conversion des clients, les marques peuvent transformer un prospect actuellement à l’étape de la considération en client converti. Les outils d’optimisation de conversion connectés analysent l’expérience de chaque prospect un par un et améliorent leur expérience unique en conséquence. En vous concentrant sur le parcours de chaque utilisateur et en mettant l’accent sur le succès individuel de chaque client, vous pouvez :

* Analysez tous les moteurs contextuels des expériences attrayantes à la fois par le biais de données de caractéristiques et de comportements afin d’améliorer les événements de conversion.
* Utilisez des outils de segmentation et d’activation avancés pour les profils pseudonymes qui n’ont pas encore été enregistrés ou convertis.
* Mettez à l’échelle les expériences individuelles dans les domaines de la publicité, des réseaux sociaux, des e-mails, du texte, etc., en utilisant les intérêts et les canaux préférés pour créer de la personnalisation.
* Utilisez la modélisation AI pour identifier les indicateurs et orienter les utilisateurs vers le meilleur chemin, comme des essais gratuits ou des conversations en ligne.

L’optimisation de la conversion des clients est une question d’équilibre. Les mesures de succès à court terme (telles que le taux de conversion) sont importantes, mais les mesures à long terme (telles que la fidélité, la valeur de durée de vie, l’opinion de la marque et le score net du promoteur) sont ce qui garantit un avenir brillant et durable à votre marque.

Ainsi, au lieu d’essayer de générer le même événement de conversion à partir de larges segments, concentrez-vous sur la création de relations client individuelles qui durent, et analysez les signaux et les modèles de succès. Vos taux de conversion s’amélioreront naturellement en tant que sous-produit d’une stratégie axée sur l’expérience client.

### Objectifs, défis et différentiels

Le cas d’utilisation de l’optimisation de la conversion client répond à un certain nombre de défis courants lorsque vous tentez d’atteindre des objectifs. La combinaison des solutions d’Adobe telles que Real-time Customer Data Platform, Journey Optimizer et Customer Journey Analytics, et de leurs capacités sous-jacentes, se différencie pour surmonter ces défis.

| Intention | Défi | Différence |
|---| ---|---|
| **Créer une profondeur <br/>compréhension<br/>chaque individu<br/>parcours du prospect<br/>et expérience.** | La plupart des entreprises doivent comprendre la conversion au niveau macro (événements générateurs de recettes). Ensuite, associez cette conversion de macroniveau aux facteurs de contribution pour tout type d’événement de conversion.<br/><br/>L’optimisation de la conversion est également limitée aux canaux numériques isolés (par exemple, web ou mobile). | Adobe offre une solution plus globale et axée sur le client qui permet aux marques d’analyser tous les moteurs contextuels qui contribuent à des expériences positives et attrayantes afin d’améliorer considérablement les événements de conversion importants pour chaque client sur l’ensemble des canaux.<br/><br/>Vous pouvez agir sur les informations tout au long du parcours afin d’optimiser les expériences à l’aide de données de caractéristiques et comportementales. |
| **Identifier et analyser<br/>signaux et intention<br/>pour chaque prospect<br/>besoins de conversion.** | Difficulté pour les clients micro et nano-segmentation à lancer des campagnes ciblées pour différentes cohortes afin de stimuler l’inscription ou la conversion<br/><br/>Les marques peinent à utiliser des comportements anonymes pour planifier des expériences percutantes sur plusieurs canaux. | Adobe permet des fonctionnalités avancées de segmentation et d’activation pour les profils pseudonymes qui n’ont pas encore été enregistrés ou convertis.<br/><br/>En tant qu’utilisateur, au fil de son processus de prise en compte de la conversion, les applications Adobe peuvent travailler ensemble pour créer des expériences omnicanales connectées à travers la publicité, les réseaux sociaux, le courrier électronique, le texte, etc. |
| **Interagir et<br/>personnaliser à chaque<br/>prospect<br/>à l’échelle.** | Le manque de connaissance des interactions des clients sur d’autres canaux entraîne des messages obsolètes ou non pertinents et risque de nuire à long terme à la marque. <br/><br/>Difficulté à maintenir une expérience cohérente et connectée à mesure qu’un utilisateur passe les étapes de réflexion à la conversion | Démarrer les clients sur leur parcours avec votre marque en facilitant un marketing omnicanal personnalisé et individualisé qui reconnaît leurs intérêts et les engage dans leurs canaux préférés.<br/><br/>Utilisez la modélisation AI pour identifier intelligemment les indicateurs et orienter chaque utilisateur vers le meilleur chemin, comme un téléchargement d’essai ou un chat. |
...
L’optimisation de la conversion client, comme pour tout autre cas d’utilisation d’informations personnalisées et d’engagement, nécessite une architecture pour prendre en charge le cas d’utilisation. Voir les présentations de l’architecture pour :
...
* [Experience Platform et applications](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/architecture-overview/platform-applications),
* [Flux de données Experience Platform](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/architecture-overview/platform-data-flow),
* et son déploiement à l’aide de la fonction [SDK web/mobile Experience Platform et Edge Network](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/architecture-overview/deployment/websdk) et la variable [SDK d’application](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/architecture-overview/deployment/appsdk)

## Solutions et fonctionnalités

Pour réaliser et mettre en oeuvre vos cas d’utilisation spécifiques d’optimisation de la conversion client, pensez à utiliser les solutions d’Adobe suivantes.
...
Pour comprendre comment Customer Journey Analytics s’intègre dans l’architecture globale des solutions d’Adobe et de votre infrastructure, reportez-vous à la section
...
|---|---|---|
| Analysis Workspace pour Customer Insight. | Espace de travail flexible pour l’exploration des données profondes qui peut être adapté au rôle de l’utilisateur. Toute personne responsable du parcours client peut prendre des décisions basées sur les données, en fonction des informations détaillées fournies par le client. | <ul><li>[Vue d’ensemble d’Analysis Workspace](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-workspace/home)</li></ul> |
...
Pour comprendre comment Real-time Customer Data Platform s’intègre dans l’architecture globale des solutions d’Adobe et de votre infrastructure, reportez-vous aux plans directeurs architecturaux suivants :
...
Pour comprendre comment Journey Optimizer s’intègre dans l’architecture globale des solutions d’Adobe et de votre infrastructure, reportez-vous aux plans directeurs architecturaux suivants :
...
L’exemple ci-dessous illustre le cas d’utilisation de l’optimisation de la conversion client pour une marque fictive Luma et un client fictif nommé Sarah.

| Étape | Description | Plus d’informations |
|---|---|---|
| 1 | Sarah est membre de Luma plus qui a effectué des achats en ligne et en magasin. Récemment, elle n&#39;a vu aucun produit qui l&#39;intéresse et n&#39;a fait aucun achat. | |
| 2 | Vous souhaitez identifier un client comme Sarah. Clients qui ont déjà acheté des vêtements de yoga mais n’ont pas effectué d’achat récent. | Utilisez Customer Journey Analytics pour définir un filtre (segment) des clients qui ont acheté des produits de yoga, mais sans achat récent.<br/>Appliquez le filtre à une table à structure libre de Campaign marketing pour identifier les campagnes qui génèrent le plus de ventes pour ce groupe. |
| 3 | Vous souhaitez également vendre aux nouveaux visiteurs et prospects. | Utilisez Customer AI pour exécuter un score de propension à l’aide du modèle de vente incitative des clients pour les clients qui sont susceptibles d’acheter des vêtements de yoga et de créer un segment. Dans Real-time Customer Data Profile, passez en revue le segment et ses options. Activez le segment sur Target. |
| 4 | Sara lit ses nouvelles du matin et voit une publicité de yoga Luma. Elle accepte les cookies, clique sur la publicité et remarque que le site web est personnalisé pour elle (à l’aide de Target). Elle commence à parcourir le contenu et le contenu est progressivement personnalisé à chaque clic. | Vous utilisez Target pour créer des activités qui personnalisent l’expérience du visiteur. |
| 5 | Sarah ajoute les shorts de yoga à son panier.. et se connecte en fournissant son adresse électronique. | Vous remarquerez que Real-Time Customer Data Profile reconnaîtra Sarah (en la faisant passer d’un profil anonyme à un profil connu). |
| 6 | Pas sûre du produit, de son prix élevé, et de ne pas trouver les informations dont Sara a besoin avant d&#39;acheter le produit, elle décide d&#39;appeler le centre d&#39;appels pour obtenir de l&#39;aide. Le représentant du centre d’appel lui donne des détails supplémentaires, expliquant qu’elle est éligible pour une offre Luma plus à un cours de yoga gratuit dans son Luma voisin. | Vous utilisez l’application du centre d’appels, intégrée à Real-time Customer Data Platform, pour trouver toutes les informations pertinentes sur Sarah pendant la conversation. <br/>Vous utilisez la gestion des décisions pour configurer les décisions, emplacements et offres. |
| 7 | Armé de plus d’informations de la part de l’agent du centre d’appel et maintenant de 10 % de réduction, Sarah décide de terminer l’achat. Elle reçoit une confirmation par e-mail indiquant les informations sur le produit acheté, le statut mis à jour du point de fidélité et l’offre pour un cours de yoga gratuit sur son blog Luma voisin. | Vous utilisez la visionneuse de profils dans Real-time Customer Data Platform pour afficher le profil de Sarah, ainsi que tous les nouveaux segments auxquels elle est désormais admissible. <br/>Vous utilisez Journey Optimizer pour créer le parcours qui envoie l’email de confirmation lors de l’achat.<br/>Vous utilisez le gestionnaire de décision pour définir les règles qui envoient l’offre spécifique dans le cadre de l’e-mail de confirmation. |
| ... | etc. | |
