...
Une vue de données est liée à une Customer Journey Analytics [Connexion](/help/connections/overview.md). Et une connexion est basée sur un sandbox au sein de votre organisation. Une fois la configuration enregistrée, le **[!UICONTROL Sandbox]** est automatiquement renseigné avec le nom correct du sandbox, en fonction de la vue de données sélectionnée.
...
* Pour créer une propriété Tags :
  ...
  ![Nouvelle Balise De Collecte De Données Content Analytics](../assets/aca-configuration-datacollection-newtag.png)
   ...
   2. Spécifiez un **[!UICONTROL Nom des balises]** par exemple `ACA Test`.
...
## Paramètres et configurations de l’intégration

Les sections suivantes décrivent les paramètres et configurations appliqués à [Customer Journey Analytics](#customer-journey-analytics-cja), [Experience Platform](#experience-platform-aep) et [Collecte de données](#data-collection-dc) dans le cadre de l’implémentation d’une configuration Content Analytics.
