...
Ce modèle de fonction extrait les valeurs de chaîne de requête pour le paramètre de requête spécifié. Le paramètre de requête respecte la casse. Insérez une fonction [Minuscules](#lowercase) pour vous assurer que toutes les variations en majuscules et en minuscules du paramètre de requête sont prises en compte.
...
Ce modèle de fonction obtient la latitude pour un état des États-Unis avec une précision à 5 chiffres.
...
![Copie d’écran du créateur de règles Latitude de l’État](assets/function-template-state-latitude.png)
...
Ce modèle de fonction obtient la longitude d’un état des États-Unis avec une précision à 5 chiffres.
...
![Copie d’écran du créateur de règles Longitude de l’État](assets/function-template-state-longitude.png)
...
Pour utiliser le modèle, vous devez spécifier les paramètres de chaque fonction répertoriée dans le cadre des règles du modèle. Supprimez des fonctions (par exemple, [URL d’analyse](#url-parse)) ou des paramètres dans des fonctions (par exemple, [Concaténer](#concatenate) et [Cas où](#case-when)) pour les paramètres de requête UTM que vous n’utilisez pas. Consultez [Référence de fonction](#function-reference) pour en savoir plus.

![Capture d’écran du créateur de règles d’analyse des paramètres UTM](assets/function-template-utm-parameters-parse.png)
...
En tant qu’analyste marketing d’une société hôtelière, vous souhaitez comprendre la différence du nombre de jours entre les dates d’enregistrement de la clientèle et les dates de réservation au cours de la semaine écoulée.
...
En tant qu’analyste marketing d’une boutique physique, vous souhaitez comprendre combien de jours se sont écoulés depuis la dernière visite d’un client ou d’une cliente dans la boutique. Vous utilisez la fonctionnalité de géolocalisation dans une application mobile et des balises dans la boutique pour capturer les visites physiques.

### Champs dérivés {#datemath-uc2-derivedfield}

Vous définissez un nouveau champ dérivé `Days Since Visit To Shop`. Utilisez la fonction [!UICONTROL DATE MATH] pour définir une règle afin de calculer le nombre de jours entre une date-heure personnalisée (que vous spécifiez dans [!UICONTROL Date]) et l’[!UICONTROL heure locale] (à partir du groupe de champs [!UICONTROL placeContext] de votre jeu de données d’événement) avec une [!UICONTROL portée de déduplication] de [!UICONTROL Personne]. Sélectionnez [!UICONTROL Renvoyer la dernière valeur] pour vous assurer que la valeur étendue de la dernière personne pour [!UICONTROL Heure locale] est utilisée dans le calcul. Vous sélectionnez Jour comme [!UICONTROL granularité de sortie].

![Copie d’écran de la règle Date Math 2](assets/datemath-2.png)
...
![Copie d’écran de la règle Date Math 2a](assets/datemath-2a.png)
...
Les options [!UICONTROL Renvoyer le premier] ou [!UICONTROL Renvoyer le dernier] ne sont pas disponibles lorsque vous sélectionnez un champ basé sur une personne (à partir d’un jeu de données de profil). Un champ basé sur la personne ne peut comporter qu’une seule valeur pour un champ de type Date ou Date-heure par personne.
+++
...
Vous définissez un nouveau champ dérivé `Internal Search Depth`. Utilisez la fonction [!UICONTROL PROFONDEUR] pour définir une règle afin de récupérer la profondeur du [!UICONTROL terme de recherche interne] et de la stocker dans un nouveau champ dérivé.

![Capture d’écran de la règle Profondeur](assets/depth-1.png)

Utilisez ensuite ce nouveau champ dérivé dans une visualisation pour ventiler selon le terme qui a été utilisé pour effectuer la première recherche.
...
Vous définissez un champ dérivé `Screen Height`. Utilisez la fonction [!UICONTROL TYPECAST] pour définir une règle afin de [!UICONTROL convertir en] [!UICONTROL Chaîne] le champ [!UICONTROL Hauteur d’écran] et de le stocker dans le nouveau champ dérivé.

![Capture d’écran de la règle 1 Typecast](assets/typecast-1.png)
...
Vous définissez un champ dérivé `Revenue (integer)`. Utilisez la fonction [!UICONTROL TYPECAST] pour définir une règle afin de [!UICONTROL convertir en] [!UICONTROL Entier] le champ [!UICONTROL Revenus] et de le stocker dans le nouveau champ dérivé.
...
| <p>Partage</p> | <ul><li>2 fonctions Partage par champ dérivé</li><li>10 valeurs maximum sont renvoyées</ul> |
