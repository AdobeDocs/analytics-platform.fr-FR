...
1. Dans la boîte de dialogue [!UICONTROL **New full table export**] , spécifiez les informations suivantes :
   ...
   | Tableau des données | Affiche le tableau à structure libre que vous exportez. Vous pouvez modifier le tableau de données en faisant glisser des composants du panneau de gauche vers le tableau. Le tableau est mis à jour dynamiquement à mesure que vous ajoutez des composants à la zone de travail.  <p>Les segments appliqués au tableau complet du projet apparaissent en haut de chaque colonne du tableau.</p> |
